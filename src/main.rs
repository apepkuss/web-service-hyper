...
async fn main() {
    ...
    router.post("/v1/completions", Box::new(handler::completions_handler));
    router.post(
        "/v1/chat/completions",
        Box::new(handler::chat_completions_handler),
    );
    router.post("/v1/embeddings", Box::new(handler::embeddings_handler));
